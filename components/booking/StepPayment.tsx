    ...
    <div className="space-y-6">
      ...
      <section className="bg-white p-6 rounded-xl shadow-sm border border-stone-200">
        ...
        <div className="space-y-4 text-sm mb-6">
          <div className="grid grid-cols-2 gap-2 bg-stone-50 p-4 rounded-lg">
            ...
            <span className="font-bold break-all">{data.guestEmail || '—'}</span>
          ...
          <div className="grid grid-cols-2 gap-2 bg-stone-50 p-4 rounded-lg">
            ...
            <span className="font-bold">{data.vehicleCount}台 ({data.vehicleCount}区画)</span>
            <span className="text-gray-500">人数</span>
            <span className="font-bold">
              大人 {data.adults}名{data.children > 0 ? ` / 子ども ${data.children}名` : ''}{data.pets > 0 ? ` / ペット ${data.pets}匹` : ''}
            </span>
          </div>
          ...
          {breakdown.length > 0 && (
            <div className="bg-stone-50 p-4 rounded-lg space-y-2">
              ...
              <div className="border-t pt-2 flex justify-between font-bold text-base">
                <span>合計 (税込)</span>
                <span className="text-[#2D4030]">¥{total.toLocaleString()}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <section className="bg-white p-6 rounded-xl shadow-sm border border-stone-200">
        ...
        <div className="flex flex-col items-center justify-center py-12 bg-stone-50 rounded-xl border-2 border-dashed border-stone-300 text-center">
          ...
          <p className="font-bold text-stone-600 mb-1">Stripe 決済 (実装予定)</p>
          <p className="text-sm text-stone-400 max-w-xs">
            Phase 2でStripe Checkoutを統合します。クレジットカード・デビットカードに対応予定です。
          </p>
        </div>
      </section>
    </div>
