...
export default function ReservationCalendar() {
  ...
  function getCellStyle(date: Date, isCurrentMonth: boolean) {
    ...
    if (!isCurrentMonth) return { cell: 'bg-white p-2 h-20 flex flex-col items-center opacity-20', text: '', spots: '' }
      ...
      <div className="grid grid-cols-7 gap-px bg-gray-200">
        {allCells.map((date, idx) => {
          ...
          return (
            ...
            >
              <span className={`text-sm ${style.text} ${date.getDay() === 0 && isCurrentMonth ? 'text-red-500' : ''} ${date.getDay() === 6 && isCurrentMonth ? 'text-blue-500' : ''}`}>
                {date.getDate()}
              </span>
              {style.spotsLabel && (
