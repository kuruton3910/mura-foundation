...
import { reservationSchema, defaultFormValues, type ReservationFormData } from '@/lib/booking/schema'
...
function validateStep1(data: ReservationFormData): string | null {
  ...
  if (data.checkoutDate <= data.checkinDate) return 'チェックアウト日はチェックイン日より後の日付を選択してください。'
...
function validateStep3(data: ReservationFormData): string | null {
  ...
  const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRe.test(data.guestEmail)) return '有効なメールアドレスを入力してください。'
  const phoneRe = /^[\d\-+() ]{10,}$/
  if (!phoneRe.test(data.guestPhone.replace(/\s/g, ''))) return '有効な電話番号を入力してください（10桁以上）。'
  return null
}
        ...
        <main className="container mx-auto px-4 pb-20">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-2/3 space-y-8">
              {currentStep === 1 && <StepConditions error={stepError ?? undefined} />}
              {currentStep === 2 && <StepTerms error={stepError ?? undefined} />}
              {currentStep === 3 && <StepPersonalInfo error={stepError ?? undefined} />}
