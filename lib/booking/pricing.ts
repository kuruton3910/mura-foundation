...
export function calcNights(checkin: Date | null, checkout: Date | null): number {
...
export function calcTotal(data: ReservationFormData): number {
  ...
  const tentFee = data.rentalTent ? data.rentalTentCount * PRICES.rentalTent : 0
  const firepitFee = data.rentalFirepit ? data.rentalFirepitCount * PRICES.rentalFirepit : 0
